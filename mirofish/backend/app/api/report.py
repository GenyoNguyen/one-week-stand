...
def generate_report():
    ...
    请求（JSON）：
        {
            ...
            "structured_table": true         // 可选，按table_schema.json生成汇总表
        }
    ...
    try:
        ...
        structured_table = data.get('structured_table', Config.REPORT_FORCE_STRUCTURED_TABLE)
        if isinstance(structured_table, str):
            structured_table = structured_table.lower() in ('true', '1', 'yes')
        ...
        if not force_regenerate:
            ...
            has_requested_output = (
                not structured_table
                or bool(existing_report and existing_report.structured_output)
            )
            if (
                existing_report
                and existing_report.status == ReportStatus.COMPLETED
                and has_requested_output
            ):
                return jsonify({
                    "success": True,
                    "data": {
        ...
        def run_generate():
            ...
            try:
                ...
                agent = ReportAgent(
                    ...
                    structured_table_required=bool(structured_table)
                )
