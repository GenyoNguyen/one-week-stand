...
- search_data_files: 从当前项目上传的PDF、CSV、JSON、Markdown和文本文件中检索原始证据
- create_structured_table: 按用户在table_schema.json中定义的列和类型验证并创建表格
...
class ReportAgent:
    ...
    def __init__(
        ...
        self.structured_table_required = (
            ...
            else structured_table_required
        )
        ...
        if self.mcp_client:
            try:
                self.mcp_tools = {
                    tool["name"]: tool for tool in self.mcp_client.list_tools()
                }
            except Exception as exc:
                logger.warning(f"Local MCP tool discovery failed: {exc}")
    ...
    def _define_tools(self) -> Dict[str, Dict[str, Any]]:
        ...
        for name, tool in self.mcp_tools.items():
            ...
            for param_name, param_schema in schema.get("properties", {}).items():
                ...
                parameters[param_name] = f"{param_type}, {requirement}. {description}".strip()
            tools[name] = {
                "name": name,
                "description": tool.get("description", "Local MCP tool"),
                "parameters": parameters,
            }
        return tools
    ...
    def _execute_tool(self, tool_name: str, parameters: Dict[str, Any], report_context: str = "") -> str:
        ...
        try:
            if self.mcp_client and tool_name in self.mcp_tools:
                arguments = dict(parameters)
                if tool_name == "search_data_files" and self.project_id:
                    arguments["project_id"] = self.project_id
                result = self.mcp_client.call_tool(tool_name, arguments)
                if tool_name == "create_structured_table":
                    table_data = json.loads(result)
                    markdown = table_data.get("markdown")
                    if markdown:
                        self.generated_tables.append(markdown)
                return result
            ...
            # ========== 向后兼容的旧工具（内部重定向到新工具） ==========
                ...
                return f"未知工具: {tool_name}。请使用以下工具之一: {', '.join(self.tools)}"
    ...
    def plan_outline(
        ...
        try:
            ...
            if len(sections) < 2:
                raise ValueError("Report outline must contain 2-5 titled sections")
    ...
    def _generate_structured_table(self, report_content: str) -> Optional[Dict[str, Any]]:
        """Convert completed report evidence into the configured table schema."""
        if not self.mcp_client or "create_structured_table" not in self.mcp_tools:
            logger.warning("Structured table requested, but create_structured_table MCP tool is unavailable")
            return None

        try:
            with open(Config.MCP_TABLE_SCHEMA_PATH, 'r', encoding='utf-8') as schema_file:
                schema = json.load(schema_file)
        except Exception as exc:
            logger.error(f"Failed to load structured table schema: {exc}")
            return None

        schema_json = json.dumps(schema, ensure_ascii=False, indent=2)
        sections = [part for part in re.split(r'(?=^##\s)', report_content, flags=re.MULTILINE) if part]
        section_budget = max(4000, 40000 // max(len(sections), 1))
        condensed_sections = []
        ...
        for _ in range(2):
            try:
                ...
                if (
                    table_output.get("type") != "structured_table"
                    or not isinstance(table_output.get("columns"), list)
                    or not table_output.get("columns")
                    or not isinstance(table_output.get("rows"), list)
                    or not 1 <= len(table_output.get("rows", [])) <= 20
                    or not isinstance(table_output.get("markdown"), str)
                    or not table_output["markdown"].strip()
                ):
                    raise ValueError("MCP tool returned an invalid structured table payload")
                return table_output
            except Exception as exc:
    ...
    def generate_report(
        ...
        try:
            ...
            if self.structured_table_required and outline.sections:
                table_output = self._generate_structured_table("\n\n".join(generated_sections))
                if not table_output:
                    raise RuntimeError("Structured table generation was required but failed")
                ...
                if table_markdown and table_markdown not in last_section.content:
                    ...
                    ReportManager.save_section(report_id, total_sections, last_section)
                    if self.report_logger:
                        self.report_logger.log_section_full_complete(
                            section_title=last_section.title,
                            section_index=total_sections,
                            full_content=f"## {last_section.title}\n\n{last_section.content}".strip()
                        )
