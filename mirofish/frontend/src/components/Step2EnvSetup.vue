    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 4 }">
        ...
        <div class="card-content">
              ...
              <div v-if="useCustomRounds" class="rounds-content custom" key="custom">
                <div class="slider-display">
                  ...
                  <div class="slider-meta-info">
                    <span>{{ $t('step2.activeAgentsSummary', { count: maxActiveAgents }) }}</span>
                  </div>
                </div>
                  ...
                  <div class="auto-content">
                    <div class="auto-meta-row">
                      <span class="duration-badge">
                        ...
                        {{ $t('step2.activeAgentsSummary', { count: maxActiveAgents }) }}
                      </span>
                    </div>
                    <div class="auto-desc">
          ...
          <div v-if="totalAgentCount" class="agent-limit-section">
            <div class="rounds-header">
              <div class="header-left">
                <span class="section-title">{{ $t('step2.activeAgentLimit') }}</span>
                <span class="section-desc">{{ $t('step2.activeAgentLimitDesc', { total: totalAgentCount }) }}</span>
              </div>
            </div>
            <div class="slider-display">
              <div class="slider-main-value">
                <span class="val-num">{{ maxActiveAgents }}</span>
                <span class="val-unit">{{ $t('step2.agentsPerRound') }}</span>
              </div>
            </div>
            <div class="range-wrapper">
              <input
                type="range"
                v-model.number="maxActiveAgents"
                min="1"
                :max="totalAgentCount"
                step="1"
                class="minimal-slider"
                :style="{ '--percent': ((maxActiveAgents - 1) / Math.max(totalAgentCount - 1, 1)) * 100 + '%' }"
              />
              <div class="range-marks">
                <span>1</span>
                <span>{{ totalAgentCount }}</span>
              </div>
            </div>
          </div>
...
const totalAgentCount = computed(() => simulationConfig.value?.agent_configs?.length || profiles.value.length)

watch(() => simulationConfig.value?.agent_configs?.length, (count) => {
  if (count > 0) {
    maxActiveAgents.value = count
  }
}, { immediate: true })
...
const handleStartSimulation = () => {
  // 构建传递给父组件的参数
  const params = { maxActiveAgents: maxActiveAgents.value }
