...
项目级 `../run.sh` 会读取父仓库的 `.env`。可通过 `MIROFISH_ENV_FILE`
显式指定其他位置；直接启动后端时，本地 `mirofish/.env` 的优先级更高。
...
每次正常启动都会先停止上一个由脚本管理的实例。使用 `../run.sh --setup-only`
仅安装依赖，使用 `../run.sh --skip-install` 快速重启，或使用
`../run.sh --stop` 停止服务。
